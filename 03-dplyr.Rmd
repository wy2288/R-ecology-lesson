...
> ### Learning Objectives
>
> * Understand the functions of the **`dplyr`** and **`tidyr`** packages.
> * Select certain columns in a data frame with the **`dplyr`** function `select`.
> * Select certain rows in a data frame with the **`dplyr`** function `filter` .
> * Link the output of one **`dplyr`** function to the input of another **`dplyr`** function with the 'pipe' operator `%>%`.
> * Add new columns to a data frame based on manipulations of existing columns with the **`dplyr`** function `mutate`.
> * Use the split-apply-combine concept for data analysis.
> * Use the **`dplyr`** functions `summarize`, `group_by`, and `count` to split a data frame into groups of observations, apply a summary statistics for each group, and then combine the results. (note: this line could be indented, because it's a description of line 26)
> * Describe what are a wide and a long table formats and why those formats are useful.
> * Describe what key-value pairs are.
> * Reshape a data frame from long to wide format and back with the `spread` and `gather` functions from the **`tidyr`** package.
